...
{
printf("reading event from vf48...\n");
  int   markerfound, j;
...
int vf48_Reset(MVME_INTERFACE *mvme, DWORD base)
{
  ...
  if (fwrev == 0x4d77d1fb) {
     ...
     if (!link_ok) {
       printf("vf48_Reset: Module 0x%06x: Problem with LVDS links, rebooting the FPGAs!\n", base);
       vf48_ReconfigureFPGA(mvme, base);
       printf("vf48_Reset: Module 0x%06x: FPGA rebooted, please run this program again!\n", base);
       exit(1);
     }
  }
